export const eventsConfig = {
  "main": [
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Best Paper Competition - Short Papers",
      "session_chair": "Sabine Timpf, Rob Lemmens",
      "track": "",
      "theme": "presentation-f57f17",
      "title": "A graph-based community detection approach for identifying the semantic neighbourhoods within London’s Airbnb properties",
      "author": [
        "Joseph Shingleton",
        "Yunus Serhat Bıçakçı",
        "Ana Basiri",
        "Meiliu Wu"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-19T11:30",
      "time_end": "2026-06-19T11:50"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Best Paper Competition - Full Papers",
      "session_chair": "Sabine Timpf, Rob Lemmens",
      "track": "",
      "theme": "presentation-f57f17",
      "title": "Towards Fine-grained Relevance Scoring of Social Media Posts in Disaster Response: A Decay-based Regression Approach",
      "author": [
        "David Hanny",
        "Andreas Kramer",
        "Ehsaneddin Jalilian",
        "Sebastian Schmidt",
        "Bernd Resch"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-19T10:50",
      "time_end": "2026-06-19T11:10"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Best Paper Competition - Full Papers",
      "session_chair": "Sabine Timpf, Rob Lemmens",
      "track": "",
      "theme": "presentation-f57f17",
      "title": "OpenStreetMap Suitability Analysis for Wheelchair Routing",
      "author": [
        "Ivan Majic",
        "Thomas Gegenleithner",
        "Johannes Scholz"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-19T11:10",
      "time_end": "2026-06-19T11:30"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Best Paper Competition - Full Papers",
      "session_chair": "Sabine Timpf, Rob Lemmens",
      "track": "",
      "theme": "presentation-f57f17",
      "title": "A Method for Spatializing Disturbance by Detecting Human-Wildlife Encounters from GNSS Trajectories",
      "author": [
        "John Dawson",
        "Veronika Peralta",
        "Ana-Maria Olteanu-Raimond",
        "Thomas Devogele",
        "Mathieu Garel"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-19T10:30",
      "time_end": "2026-06-19T10:50"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Best Paper Competition - Short Papers",
      "session_chair": "Sabine Timpf, Rob Lemmens",
      "track": "",
      "theme": "presentation-f57f17",
      "title": "Counterfactual Modelling for Evaluating Pipeline Replacement Strategies",
      "author": [
        "Nienke Maas",
        "Roel Diemel",
        "Daan van Os",
        "Judith Verstegen"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-19T11:50",
      "time_end": "2026-06-19T12:10"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Best Paper Competition - Short Papers",
      "session_chair": "Sabine Timpf, Rob Lemmens",
      "track": "",
      "theme": "presentation-f57f17",
      "title": "A High-Performance Game Engine-GIS-CFD System for Interactive Urban Wind Decision Support",
      "author": [
        "Xuanchen Zhou",
        "Azarakhsh Rafiee",
        "Peter van Oosterom"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-19T12:10",
      "time_end": "2026-06-19T12:30"
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Coffee Break",
      "location": "Mart Reiniku Building",
      "time_start": "2026-06-17T10:30",
      "time_end": "2026-06-17T11:00"
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Break",
      "location": "",
      "time_start": "2026-06-17T15:00",
      "time_end": "2026-06-17T15:15"
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Coffee Break",
      "location": "Mart Reiniku Building",
      "time_start": "2026-06-18T10:30",
      "time_end": "2026-06-18T11:00"
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Coffee Break",
      "location": "Mart Reiniku Building",
      "time_start": "2026-06-18T15:40",
      "time_end": "2026-06-18T16:10"
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Coffee Break",
      "location": "Mart Reiniku Building",
      "time_start": "2026-06-19T10:00",
      "time_end": "2026-06-19T10:30"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Ecology & Sustainable Systems Modelling",
      "session_chair": "Johannes Scholz",
      "track": "track1",
      "theme": "presentation-3c7d22",
      "title": "Behavior Analysis of Rhinoceroses and their Monitoring using a 3D Geovisualization Prototype",
      "author": [
        "Martine Besse",
        "Basile Thullen",
        "Maryam Lotfian",
        "Vanessa Duthé",
        "Siphesihle Mbongwa",
        "Jürgen Ehrensberger",
        "Jens Ingensand"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T13:40",
      "time_end": "2026-06-17T14:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Ecology & Sustainable Systems Modelling",
      "session_chair": "Johannes Scholz",
      "track": "track1",
      "theme": "presentation-3c7d22",
      "title": "Discovering environmental health effects of transport scenarios through agent-based simulations",
      "author": [
        "Tabea Sonnenschein",
        "Simon Scheider",
        "Ardine de Wit",
        "James Woodcock",
        "Roel Vermeulen"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T14:20",
      "time_end": "2026-06-17T14:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Ecology & Sustainable Systems Modelling",
      "session_chair": "Johannes Scholz",
      "track": "track1",
      "theme": "presentation-3c7d22",
      "title": "From Individual Choices to Collective Impact: Quantifying the Role of Behavioural Changes in Greenhouse Gas Mitigation",
      "author": [
        "Maryam Naghdizadeganjahromi",
        "Sébastien Bruno",
        "Marius Le Maréchal",
        "Stéphane Roche"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T14:40",
      "time_end": "2026-06-17T15:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Ecology & Sustainable Systems Modelling",
      "session_chair": "Johannes Scholz",
      "track": "track1",
      "theme": "presentation-3c7d22",
      "title": "A new data-driven paradigm for the study of avian migratory navigation",
      "author": [
        "Urska Demsar"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T14:00",
      "time_end": "2026-06-17T14:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Education track",
      "session_chair": "Jing Yao",
      "track": "track2",
      "theme": "presentation-e67e22",
      "title": "Reframing Geographic Information Science Education: Integrating Flipped Classrooms and GenAI for Active and Critical Learning",
      "author": [
        "Vicente Tang",
        "Marco Painho"
      ],
      "location": "Room 327",
      "time_start": "2026-06-17T13:40",
      "time_end": "2026-06-17T14:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Education track",
      "session_chair": "Jing Yao",
      "track": "track2",
      "theme": "presentation-e67e22",
      "title": "A dual-language framework for inclusive teaching of Geographic Data Science in diverse student cohorts",
      "author": [
        "Carmen Cabrera",
        "Elisabetta Pietrostefani"
      ],
      "location": "Room 327",
      "time_start": "2026-06-17T14:00",
      "time_end": "2026-06-17T14:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Foundation Models & Generalisable GeoAI",
      "session_chair": "James Ackland",
      "track": "track1",
      "theme": "presentation-3d6fa3",
      "title": "Investigating the Generalizability of Segment Anything Model for Large-Scale Geospatial Segmentation",
      "author": [
        "Wejdene Mansour",
        "Paul Walther",
        "Hao Li",
        "Martin Werner"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T11:00",
      "time_end": "2026-06-18T11:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Foundation Models & Generalisable GeoAI",
      "session_chair": "James Ackland",
      "track": "track1",
      "theme": "presentation-3d6fa3",
      "title": "Assessing the Geographic Diversity of AI's Platial Representations in Image Generation",
      "author": [
        "Zilong Liu",
        "Krzysztof Janowicz",
        "Mina Karimi"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T11:20",
      "time_end": "2026-06-18T11:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Foundation Models & Generalisable GeoAI",
      "session_chair": "James Ackland",
      "track": "track1",
      "theme": "presentation-3d6fa3",
      "title": "CLIP the landscape: Automated tagging of crowdsourced landscape images",
      "author": [
        "Ilya Ilyankou",
        "Natchapon Jongwiriyanurak",
        "Tao Cheng",
        "James Haworth"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T11:40",
      "time_end": "2026-06-18T12:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Foundation Models & Generalisable GeoAI",
      "session_chair": "James Ackland",
      "track": "track1",
      "theme": "presentation-3d6fa3",
      "title": "Generalizability of Foundation Models: A Case Study on Cocoa Mapping Across Countries Using Sparse Labels",
      "author": [
        "Ruslan Mammadov",
        "Paul Walther",
        "Julius Fricke",
        "Martin Werner"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T12:00",
      "time_end": "2026-06-18T12:20"
    },
    {
      "event_group": "social",
      "event_format": "keynote",
      "track": "",
      "session_category": "Keynote",
      "session_chair": "Alexander Kmoch",
      "theme": "keynote-c00000",
      "title": "Multimodal GeoAI: User-generated Data-Enabled Sensing and Actuating",
      "author": [
        "Prof. Bernd Resch"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-19T09:00",
      "time_end": "2026-06-19T10:00"
    },
    {
      "event_group": "social",
      "event_format": "keynote",
      "track": "",
      "session_category": "Keynote",
      "session_chair": "Ali Mansourian",
      "theme": "keynote-c00000",
      "title": "Agentic AI Systems for Geospatial Intelligence in Support of EU Policy",
      "author": [
        "Pieter Kempeneers"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T09:30",
      "time_end": "2026-06-18T10:30"
    },
    {
      "event_group": "social",
      "event_format": "keynote",
      "track": "",
      "session_category": "Keynote",
      "session_chair": "Evelyn Uuemaa",
      "theme": "keynote-c00000",
      "title": "Making mobile big data actionable for spatial planning and management  – examples from Finland and beyond",
      "author": [
        "Prof. Tuuli Toivonen"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T09:30",
      "time_end": "2026-06-17T10:30"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "LiDAR & 3D Geospatial Analysis",
      "session_chair": "Martin Breunig",
      "track": "track1",
      "theme": "presentation-5d95d5",
      "title": "CascadeCBF: Probabilistic Counting for Sparse Spatial Point Clouds",
      "author": [
        "Moritz Laass",
        "Paul Walther",
        "Wejdene Mansour",
        "Martin Werner"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T11:00",
      "time_end": "2026-06-17T11:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "LiDAR & 3D Geospatial Analysis",
      "session_chair": "Martin Breunig",
      "track": "track1",
      "theme": "presentation-5d95d5",
      "title": "LidarTeam: a remote sensing driven method for massive lidar data to regional DHM refined through user feedback",
      "author": [
        "Xavier Pons",
        "Oscar González-Guerrero",
        "Joan Masó",
        "Alaitz Zabala",
        "Ivette Serral",
        "Miquel Ninyerola"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T11:20",
      "time_end": "2026-06-17T11:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "LiDAR & 3D Geospatial Analysis",
      "session_chair": "Martin Breunig",
      "track": "track1",
      "theme": "presentation-5d95d5",
      "title": "Towards National-Scale Ecological Applications: Harmonised Framework for LiDAR Point Cloud Processing for Vegetation Metrics Calculation",
      "author": [
        "Eveli Sisas",
        "Holger Virro",
        "Wai Tik Chan",
        "Alexander Kmoch",
        "Aveliina Helm",
        "Evelyn Uuemaa"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T12:20",
      "time_end": "2026-06-17T12:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "LiDAR & 3D Geospatial Analysis",
      "session_chair": "Martin Breunig",
      "track": "track1",
      "theme": "presentation-5d95d5",
      "title": "A comparison of generalised urban block 3D model variants considering attributes with an influence on urban analyses",
      "author": [
        "Alejandro Domínguez-Lapeña",
        "Rubén Béjar",
        "Ana Ruiz-Varona"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T11:40",
      "time_end": "2026-06-17T12:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "LiDAR & 3D Geospatial Analysis",
      "session_chair": "Martin Breunig",
      "track": "track1",
      "theme": "presentation-5d95d5",
      "title": "Enhancing OpenStreetMap Building Footprints through nDSM-Based Geometric Segmentation for AI Training Data",
      "author": [
        "Paul Kuper",
        "Ruiqi Liu",
        "Hanwen Deng",
        "Martin Breunig"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T12:00",
      "time_end": "2026-06-17T12:20"
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Lunch",
      "location": "Mart Reiniku Building",
      "time_start": "2026-06-17T12:40",
      "time_end": "2026-06-17T13:40"
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Lunch",
      "location": "Mart Reiniku Building",
      "time_start": "2026-06-18T12:20",
      "time_end": "2026-06-18T13:20"
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Lunch",
      "location": "Mart Reiniku Building",
      "time_start": "2026-06-19T13:10",
      "time_end": "2026-06-19T14:10"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Mobility Tracking & Travel Behaviour Analysis",
      "session_chair": "Sabine Timpf",
      "track": "track3",
      "theme": "presentation-65528f",
      "title": "Local Calibration of OpenStreetMap GPS Traces for Traffic Volume Estimation",
      "author": [
        "Michael Schötz",
        "Martina Schöll",
        "Thomas Limbrunner"
      ],
      "location": "Room 246",
      "time_start": "2026-06-17T11:40",
      "time_end": "2026-06-17T12:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Mobility Tracking & Travel Behaviour Analysis",
      "session_chair": "Sabine Timpf",
      "track": "track3",
      "theme": "presentation-65528f",
      "title": "Publication summary of 'Utilizing mobile phone tracking data to estimate Intra-City modal mobility: A study on active mobility in two Finnish City regions'",
      "author": [
        "Marton Magyar",
        "Terhi Ala-Hulkko",
        "Harri Antikainen",
        "Tiina Lankila",
        "Ossi Kotavaara"
      ],
      "location": "Room 246",
      "time_start": "2026-06-17T12:00",
      "time_end": "2026-06-17T12:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Mobility Tracking & Travel Behaviour Analysis",
      "session_chair": "Sabine Timpf",
      "track": "track3",
      "theme": "presentation-65528f",
      "title": "Beyond the Immediate Impacts of COVID-19 on Internal Population Movements in Mexico: Facebook Data Reveal Urban Decay and Slow Recovery",
      "author": [
        "Miguel González-Leonardo",
        "Carmen Cabrera",
        "Ruth Neville",
        "Andrea Nasuto",
        "Francisco Rowe"
      ],
      "location": "Room 246",
      "time_start": "2026-06-17T12:20",
      "time_end": "2026-06-17T12:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Mobility Tracking & Travel Behaviour Analysis",
      "session_chair": "Sabine Timpf",
      "track": "track3",
      "theme": "presentation-65528f",
      "title": "Integrating state-space models with map-matching to improve estimates of active travel motion",
      "author": [
        "Corin Staves",
        "Mads Paulsen",
        "Xixi Wang",
        "Laurent Cazor",
        "Thomas Rasmussen"
      ],
      "location": "Room 246",
      "time_start": "2026-06-17T11:20",
      "time_end": "2026-06-17T11:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Mobility Tracking & Travel Behaviour Analysis",
      "session_chair": "Sabine Timpf",
      "track": "track3",
      "theme": "presentation-65528f",
      "title": "spanishoddata: A package for accessing and working with Spanish Open Mobility Big Data",
      "author": [
        "Egor Kotov",
        "Eugeni Vidal-Tortosa",
        "Oliva Cantú-Ros",
        "Javier Burrieza-Galán",
        "Ricardo Herranz",
        "Tania Gullón Muñoz-Repiso",
        "Robin Lovelace"
      ],
      "location": "Room 246",
      "time_start": "2026-06-17T11:00",
      "time_end": "2026-06-17T11:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Participatory Mapping & Planning",
      "session_chair": "Age Poom",
      "track": "track3",
      "theme": "presentation-d28f7b",
      "title": "Enhancing the Potential of a Tangible-Digital Planning Interface through User Evaluation",
      "author": [
        "Vincent Holtorf",
        "Maria Moleiro Dale",
        "Jörg Rainer Noennig"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T16:10",
      "time_end": "2026-06-18T16:30"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Participatory Mapping & Planning",
      "session_chair": "Age Poom",
      "track": "track3",
      "theme": "presentation-d28f7b",
      "title": "Object-Level Detection of Hand-Drawn Annotations in Participatory Sketch Maps Using Paired Clean and Annotated Basemaps",
      "author": [
        "Clemens Langer",
        "Celina Thomé",
        "Nir Fulman",
        "Steffen Knoblauch",
        "Alexander Zipf",
        "Yulia Grinblat"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T16:30",
      "time_end": "2026-06-18T16:50"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Participatory Mapping & Planning",
      "session_chair": "Age Poom",
      "track": "track3",
      "theme": "presentation-d28f7b",
      "title": "Can Large Language Models interpret participatory mapping comments at scale? Evidence from Prague’s emotional mapping",
      "author": [
        "Lydia Valdesera",
        "Thibaud Chassin",
        "Jiří Pánek"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T16:50",
      "time_end": "2026-06-18T17:10"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Participatory Mapping & Planning",
      "session_chair": "Age Poom",
      "track": "track3",
      "theme": "presentation-d28f7b",
      "title": "Turning Geospatial Data into Planning Decisions: Evaluating a Participatory Accessibility Model for Urban Drinking Water",
      "author": [
        "Oluwatimilehin Shonowo",
        "Xingyi Du",
        "Diego Pajarito Grajales",
        "Sebastian Hafner",
        "Joao Porto de Albuquerque",
        "Qunshan Zhao",
        "Peter Elias",
        "Kehinde Baruwa",
        "Oluwatoyin Odulana",
        "Sabitu Saad",
        "Taiwo Adekanmbi",
        "Solomon Abioye Olusegun",
        "Jose Luis Fernandez-Marquez"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T17:10",
      "time_end": "2026-06-18T17:30"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Place Semantics & Spatial Knowledge Representation",
      "session_chair": "Simon Scheider",
      "track": "track3",
      "theme": "presentation-8b3a3a",
      "title": "Spatio-Temporal Knowledge Graph from Unstructured Texts: A Multi-Scale Approach for Food Security Monitoring",
      "author": [
        "Charles Abdoulaye Ngom",
        "Landy Rajaonarivo",
        "Maguelonne Teisseire",
        "Sarah Valentin"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T14:20",
      "time_end": "2026-06-18T14:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Place Semantics & Spatial Knowledge Representation",
      "session_chair": "Simon Scheider",
      "track": "track3",
      "theme": "presentation-8b3a3a",
      "title": "Ontology-based Textual Location Description Framework for Multidimensional Features",
      "author": [
        "Ting-Long Yan",
        "Chiao-Ling Kuo"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T15:20",
      "time_end": "2026-06-18T15:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Place Semantics & Spatial Knowledge Representation",
      "session_chair": "Simon Scheider",
      "track": "track3",
      "theme": "presentation-8b3a3a",
      "title": "Should we ask an LLM? Evaluating Toponym Disambiguation across Administrative Levels",
      "author": [
        "Franz Welscher",
        "Paddy Smith",
        "Tatu Leppämäki",
        "Ilya Ilyankou"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T15:00",
      "time_end": "2026-06-18T15:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Place Semantics & Spatial Knowledge Representation",
      "session_chair": "Simon Scheider",
      "track": "track3",
      "theme": "presentation-8b3a3a",
      "title": "Geo-Alignment of Vague Cognitive Regions: Representing Uneven Cognitive Geographies of Large Language Models",
      "author": [
        "Mina Karimi",
        "Krzysztof Janowicz",
        "Zilong Liu",
        "Songlin Wang",
        "Annika Süß"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T14:40",
      "time_end": "2026-06-18T15:00"
    },
    {
      "event_group": "social",
      "event_format": "opening",
      "track": "",
      "session_category": "Posters, Networking, Welcome, Closing",
      "session_chair": "",
      "theme": "opening-ffc000",
      "title": "Welcome",
      "author": [],
      "location": "Ring Auditorium",
      "time_start": "2026-06-17T09:00",
      "time_end": "2026-06-17T09:30"
    },
    {
      "event_group": "social",
      "event_format": "posters",
      "track": "",
      "session_category": "Posters, Networking, Welcome, Closing",
      "session_chair": "",
      "theme": "networking-ffc000",
      "title": "Posters",
      "author": [],
      "location": "Ring Auditorium/Mart Reiniku Building",
      "time_start": "2026-06-17T15:15",
      "time_end": "2026-06-17T18:00"
    },
    {
      "event_group": "social",
      "event_format": "networking",
      "track": "track3",
      "session_category": "Posters, Networking, Welcome, Closing",
      "theme": "networking-ffc000",
      "title": "Networking Session",
      "location": "Room 246",
      "time_start": "2026-06-17T13:40",
      "time_end": "2026-06-17T15:00",
      "description": "Join the Networking Session for an interactive speed-networking experience where researchers can connect through short one-on-one conversations, rotating topic tables, and opportunities to connect with researchers across shared interests. Your next collaborator, mentor, or co-author could be just one conversation away."
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Public Health & Urban Vulnerability",
      "session_chair": "Carsten Keßler",
      "track": "track2",
      "theme": "presentation-753131",
      "title": "Trends and distribution of drug-related and associated events: A Case Study of Vilnius",
      "author": [
        "Gordonas Jankūnas"
      ],
      "location": "Room 327",
      "time_start": "2026-06-17T11:20",
      "time_end": "2026-06-17T11:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Public Health & Urban Vulnerability",
      "session_chair": "Carsten Keßler",
      "track": "track2",
      "theme": "presentation-753131",
      "title": "Analysis of the spatial and temporal pattern of COVID-19 incidence rate across Germany",
      "author": [
        "Sven Lautenbach",
        "Marcel Maurer",
        "Alexander Zipf"
      ],
      "location": "Room 327",
      "time_start": "2026-06-17T11:40",
      "time_end": "2026-06-17T12:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Public Health & Urban Vulnerability",
      "session_chair": "Carsten Keßler",
      "track": "track2",
      "theme": "presentation-753131",
      "title": "Food Insecurity Projections for Anticipatory Action: Comparative Spatiotemporal Analysis of FEWS NET and the IPC in Somalia",
      "author": [
        "Ferdinand Seyffer",
        "Anne Schauss",
        "Suella Tirai",
        "Marcel Maurer",
        "Sven Lautenbach",
        "Alexander Zipf"
      ],
      "location": "Room 327",
      "time_start": "2026-06-17T12:00",
      "time_end": "2026-06-17T12:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Public Health & Urban Vulnerability",
      "session_chair": "Carsten Keßler",
      "track": "track2",
      "theme": "presentation-753131",
      "title": "Detour-based metrics of alcohol availability along walking commutes",
      "author": [
        "Nir Fulman",
        "Abdulkadir Memduhoğlu",
        "Johannes Huber",
        "Alexander Zipf"
      ],
      "location": "Room 327",
      "time_start": "2026-06-17T11:00",
      "time_end": "2026-06-17T11:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Public Health & Urban Vulnerability",
      "session_chair": "Carsten Keßler",
      "track": "track2",
      "theme": "presentation-753131",
      "title": "The green gap: Socio-economic disparities in the utilisation of urban green spaces in Auckland, Hong Kong, and Taipei",
      "author": [
        "I-Ting Chuang",
        "Danjie Shen",
        "Ate Poorthuis",
        "Jessie Colbert",
        "Paulina Wong",
        "Ka Shing William Cheung",
        "Desmond Tsang",
        "Hsueh Cheng-Luen",
        "Katarzyna Sila-Nowicka"
      ],
      "location": "Room 327",
      "time_start": "2026-06-17T12:20",
      "time_end": "2026-06-17T12:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Remote Sensing for Environmental Hazards",
      "session_chair": "Urska Demsar",
      "track": "track1",
      "theme": "presentation-275317",
      "title": "Monitoring of pesticide treatment effects on crops using optical and SAR satellite remote sensing",
      "author": [
        "Tatjana Kuznecova",
        "Gustavo Garcia Chapeton"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T16:30",
      "time_end": "2026-06-18T16:50"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Remote Sensing for Environmental Hazards",
      "session_chair": "Urska Demsar",
      "track": "track1",
      "theme": "presentation-275317",
      "title": "Advancing National Fire Danger Forecasting through Geoinformatics",
      "author": [
        "Valentina Sagris",
        "Tõnu Oja",
        "Merle Muru"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T17:10",
      "time_end": "2026-06-18T17:30"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Remote Sensing for Environmental Hazards",
      "session_chair": "Urska Demsar",
      "track": "track1",
      "theme": "presentation-275317",
      "title": "Validation of SWAT+ Hydrological Model Using Remote Sensing-Based  Evapotranspiration Data",
      "author": [
        "Essmat A. A. Mohamed",
        "Amanda Batlle Morera",
        "Lluís Pesquer",
        "Miquel Ninyerola"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T16:50",
      "time_end": "2026-06-18T17:10"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Remote Sensing for Environmental Hazards",
      "session_chair": "Urska Demsar",
      "track": "track1",
      "theme": "presentation-275317",
      "title": "Mapping forest degradation using scikit-eo. A Python Package for Remote Sensing Data Analysis",
      "author": [
        "Fernando Benitez-Paez",
        "Yonatan Tarazona Coronel"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T16:10",
      "time_end": "2026-06-18T16:30"
    },
    {
      "event_group": "social",
      "event_format": "social",
      "track": "",
      "theme": "social-ffffff",
      "title": "Pub Crawl",
      "location": "Pühaste kelder | Pubi RP 9 | Vein ja Vine | Möku",
      "webpage": "",
      "directions": "https://maps.app.goo.gl/1Wt4HP1cvRzBjDJcA",
      "time_start": "2026-06-17T20:00",
      "time_end": "2026-06-17T23:00"
    },
    {
      "event_group": "social",
      "event_format": "social",
      "track": "",
      "theme": "social-ffffff",
      "title": "Guided city tour in Tartu",
      "location": "Town Hall Square, next to the fountain",
      "webpage": "",
      "directions": "https://maps.app.goo.gl/jUfxmpy7hjkiLv738",
      "time_start": "2026-06-17T19:00",
      "time_end": "2026-06-17T20:00"
    },
    {
      "event_group": "social",
      "event_format": "social",
      "track": "",
      "theme": "social-ffffff",
      "title": "Conference dinner & Boat trips on Emajõgi",
      "location": "Lodjakoda",
      "webpage": "",
      "directions": "https://maps.app.goo.gl/3DPtEEDgwQ5oJkfg9",
      "time_start": "2026-06-18T19:00",
      "time_end": "2026-06-18T23:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Spatial Data Infrastructure & Scalable Analytics",
      "session_chair": "Juha Oksanen",
      "track": "track2",
      "theme": "presentation-2e5c8a",
      "title": "From Missingness to Motivation: A “Living Dataset” Perspective  on Volunteering Geographic Information",
      "author": [
        "James Ackland",
        "Ana Basiri"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T16:10",
      "time_end": "2026-06-18T16:30"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Spatial Data Infrastructure & Scalable Analytics",
      "session_chair": "Juha Oksanen",
      "track": "track2",
      "theme": "presentation-2e5c8a",
      "title": "Uncertainty in Binned Building Construction Year Data: Comparing EPC and Crowdsourced Datasets",
      "author": [
        "Sophie Teichmann",
        "Polly Hudson",
        "Mihyun Kim",
        "Hendrik Herold",
        "Robert Hecht"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T16:30",
      "time_end": "2026-06-18T16:50"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Spatial Data Infrastructure & Scalable Analytics",
      "session_chair": "Juha Oksanen",
      "track": "track2",
      "theme": "presentation-2e5c8a",
      "title": "A Fractional Raster–Vector Clipping Operator for Boundary-Consistent Multi-Scale Spatial Analysis",
      "author": [
        "Younes Hamdani",
        "Urs Albert Treier",
        "Signe Normand"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T16:50",
      "time_end": "2026-06-18T17:10"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Spatial Data Infrastructure & Scalable Analytics",
      "session_chair": "Juha Oksanen",
      "track": "track2",
      "theme": "presentation-2e5c8a",
      "title": "DGGS-Native Data Cubes: A Design Pattern for Scalable, Distortion-Aware Analytics",
      "author": [
        "Michael Zamudio-Monserratt",
        "Alexander Kmoch",
        "Evelyn Uuemaa"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T17:10",
      "time_end": "2026-06-18T17:30"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Spatial Machine Learning & Predictive Analytics",
      "session_chair": "Henrikki Tenkanen",
      "track": "track1",
      "theme": "presentation-4d82bc",
      "title": "“Flags of Deceit:” Re-Flagging Trends among Sanctioned Ships in the Russian Shadow Fleet",
      "author": [
        "Mikael Brunila"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T15:20",
      "time_end": "2026-06-18T15:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Spatial Machine Learning & Predictive Analytics",
      "session_chair": "Henrikki Tenkanen",
      "track": "track1",
      "theme": "presentation-4d82bc",
      "title": "The value of where – quantifying location values with tabular transformer models",
      "author": [
        "Christian Mueller-Kett"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T14:20",
      "time_end": "2026-06-18T14:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Spatial Machine Learning & Predictive Analytics",
      "session_chair": "Henrikki Tenkanen",
      "track": "track1",
      "theme": "presentation-4d82bc",
      "title": "When Today’s Accuracy Fails Tomorrow: Evaluating Spatially Driven Real Estate Prediction Models",
      "author": [
        "Christopher Kmen",
        "Gerhard Navratil",
        "Ioannis Giannopoulos"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T14:40",
      "time_end": "2026-06-18T15:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Spatial Machine Learning & Predictive Analytics",
      "session_chair": "Henrikki Tenkanen",
      "track": "track1",
      "theme": "presentation-4d82bc",
      "title": "Adapting machine learning for environmental spatial data - A review",
      "author": [
        "Marta Jemeljanova",
        "Alexander Kmoch",
        "Evelyn Uuemaa"
      ],
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T15:00",
      "time_end": "2026-06-18T15:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Street-Level Vision & Urban Environmental Perception",
      "session_chair": "Frank Ostermann",
      "track": "track2",
      "theme": "presentation-85929e",
      "title": "Enhancing arcade detection using bi-directed scene graphs with graph convolutional networks on street view imagery",
      "author": [
        "Yi-Wen Mo",
        "Chiao-Ling Kuo",
        "Zhi-Sheng Lin"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T14:20",
      "time_end": "2026-06-18T14:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Street-Level Vision & Urban Environmental Perception",
      "session_chair": "Frank Ostermann",
      "track": "track2",
      "theme": "presentation-85929e",
      "title": "Assessing Perceived Walkability: a case study using eye-tracking and think-aloud with street images",
      "author": [
        "Chenyu Zuo",
        "Fabian Stöckl",
        "Elisa Stegmeier",
        "Jiale Xu",
        "Jonas Olbrich",
        "Leo Glavinić",
        "Teresa Haselmayr",
        "Eva Nuhn"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T14:40",
      "time_end": "2026-06-18T15:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Street-Level Vision & Urban Environmental Perception",
      "session_chair": "Frank Ostermann",
      "track": "track2",
      "theme": "presentation-85929e",
      "title": "Measuring Urban Green Space Vitality through Multi-Source Visual and Textual Data: Integrating Social Media and Street-Level Imagery in London",
      "author": [
        "Fangyuan Li",
        "Rui Wang",
        "Yijing Li",
        "Zahratu Shabrina"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T15:00",
      "time_end": "2026-06-18T15:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Street-Level Vision & Urban Environmental Perception",
      "session_chair": "Frank Ostermann",
      "track": "track2",
      "theme": "presentation-85929e",
      "title": "Maximizing Data Coverage: Fusing Street View and Oblique Imagery to Quantify Vertical Greenery Potential in Urban Areas",
      "author": [
        "Aruscha Kramm",
        "André Ludwig",
        "Bogdan Franczyk"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T15:20",
      "time_end": "2026-06-18T15:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Transport Accessibility & Active Mobility",
      "session_chair": "Carlos Granell",
      "track": "track3",
      "theme": "presentation-7b66a8",
      "title": "Linking Public Transport Provision to Functional Regions: Evaluating Transport–Commuting Alignment in Estonia",
      "author": [
        "Martin Haamer",
        "Anto Aasa"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T11:00",
      "time_end": "2026-06-18T11:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Transport Accessibility & Active Mobility",
      "session_chair": "Carlos Granell",
      "track": "track3",
      "theme": "presentation-7b66a8",
      "title": "Inclusive Multimodal Routing: How Behavioral Constraints Shape Accessibility",
      "author": [
        "Ioanna Gogousou",
        "Manuela Canestrini",
        "Negar Alinaghi",
        "Ioannis Giannopoulos"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T11:40",
      "time_end": "2026-06-18T12:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Transport Accessibility & Active Mobility",
      "session_chair": "Carlos Granell",
      "track": "track3",
      "theme": "presentation-7b66a8",
      "title": "Fuzzy Meta Indices: A Bikeability Case Study in Augsburg",
      "author": [
        "Pablo S. Löw",
        "Jukka M. Krisp"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T12:00",
      "time_end": "2026-06-18T12:20"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Transport Accessibility & Active Mobility",
      "session_chair": "Carlos Granell",
      "track": "track3",
      "theme": "presentation-7b66a8",
      "title": "Measuring regional accessibility in urban form: A reproducible approach for urban analytics",
      "author": [
        "Mathias Jehling",
        "Lucie Kluwe",
        "Caspar Kleiner"
      ],
      "location": "Room 246",
      "time_start": "2026-06-18T11:20",
      "time_end": "2026-06-18T11:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Urban Morphology & Built Environment Analytics",
      "session_chair": "Carmen Cabrera",
      "track": "track2",
      "theme": "presentation-56616c",
      "title": "The potential of street-level and 3D views: Making public participation in urban planning more accessible to people with cognitive disabilities",
      "author": [
        "Judith Kirstein",
        "Carsten Keßler",
        "Heike Köckler",
        "Christin Kupitz",
        "Christian Walter-Klose",
        "Stefan Fleck"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T11:40",
      "time_end": "2026-06-18T12:00"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Urban Morphology & Built Environment Analytics",
      "session_chair": "Carmen Cabrera",
      "track": "track2",
      "theme": "presentation-56616c",
      "title": "Beyond distance: assessing heat vulnerability through mobility-based accessibility to cooling resources",
      "author": [
        "Muchun Li",
        "Lex Comber",
        "Arjan Gosal"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T11:20",
      "time_end": "2026-06-18T11:40"
    },
    {
      "event_group": "presentation",
      "event_format": "presentation",
      "session_category": "Urban Morphology & Built Environment Analytics",
      "session_chair": "Carmen Cabrera",
      "track": "track2",
      "theme": "presentation-56616c",
      "title": "Exploring urban polygonal representation learning for complex footprint groups",
      "author": [
        "Luisa Lo Presti",
        "Peter Mooney"
      ],
      "location": "Room 327",
      "time_start": "2026-06-18T11:00",
      "time_end": "2026-06-18T11:20"
    },
    {
      "event_group": "milestone",
      "event_format": "milestone",
      "track": "",
      "title": "End of Day 2",
      "time_start": "2026-06-18T17:40",
      "time_end": ""
    },
    {
      "event_group": "milestone",
      "event_format": "milestone",
      "track": "",
      "title": "End of Day 1",
      "time_start": "2026-06-17T18:10",
      "time_end": ""
    },
    {
      "event_group": "announcement",
      "event_format": "announcement",
      "track": "",
      "theme": "keynote-c00000",
      "title": "Announcement of AGILE 2027 Conference",
      "location": "Ring Auditorium",
      "time_start": "2026-06-19T13:00",
      "time_end": "2026-06-19T13:10"
    },
    {
      "event_group": "registration",
      "event_format": "registration",
      "track": "",
      "title": "Registration desk open",
      "location": "Vanemuise 46",
      "time_start": "2026-06-16T08:00",
      "time_end": "2026-06-16T09:00"
    },
    {
      "event_group": "registration",
      "event_format": "registration",
      "track": "",
      "title": "Registration desk open",
      "location": "Vanemuise 46",
      "time_start": "2026-06-17T08:00",
      "time_end": "2026-06-17T09:00"
    },
    {
      "event_group": "assembly",
      "event_format": "assembly",
      "track": "",
      "theme": "assembly-1a3a4a",
      "title": "AGILE Annual Assembly",
      "location": "Ring Auditorium",
      "time_start": "2026-06-18T13:20",
      "time_end": "2026-06-18T14:20"
    }
  ],
  "workshops": [
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": false,
      "track": "track1",
      "theme": "workshop-1565c0",
      "title": "MobilityBank: Building a Global Collective for Open, FAIR, and Ethical Human Mobility Data",
      "short_title": "MobilityBank",
      "author": [
        "Milad Malekzadeh",
        "Urška Demšar",
        "Henrikki Tikkanen",
        "Katarzyna Sila-Nowicka",
        "Jed Long"
      ],
      "location": "Room 327",
      "time_start": "2026-06-16T09:00",
      "time_end": "2026-06-16T10:45",
      "description": "",
      "website": "https://mobilitybank.org/"
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": true,
      "track": "track1",
      "theme": "workshop-1565c0",
      "title": "MobilityBank: Building a Global Collective for Open, FAIR, and Ethical Human Mobility Data",
      "short_title": "MobilityBank",
      "author": [
        "Milad Malekzadeh"
      ],
      "location": "Room 327",
      "time_start": "2026-06-16T11:15",
      "time_end": "2026-06-16T13:00",
      "description": "",
      "website": "https://mobilitybank.org/"
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": false,
      "track": "track2",
      "theme": "workshop-1565c0",
      "title": "Geography According to Foundation Models",
      "short_title": "Foundation Models",
      "author": [
        "Krzysztof Janowicz",
        "Alexandra Fortacz",
        "Yingjing Huang"
      ],
      "location": "Room 246",
      "time_start": "2026-06-16T09:00",
      "time_end": "2026-06-16T10:45",
      "description": "",
      "website": ""
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": true,
      "track": "track2",
      "theme": "workshop-1565c0",
      "title": "Geography According to Foundation Models",
      "short_title": "Foundation Models",
      "author": [
        "Krzysztof Janowicz",
        "Alexandra Fortacz",
        "Yingjing Huang"
      ],
      "location": "Room 246",
      "time_start": "2026-06-16T11:15",
      "time_end": "2026-06-16T13:00",
      "description": "",
      "website": ""
    },
    {
      "event_group": "workshop",
      "event_format": "tutorial",
      "is_continued": false,
      "track": "track3",
      "theme": "workshop-f57f17",
      "title": "Clear by design – A practical guide to making better maps and figures",
      "short_title": "Map Design",
      "author": [
        "Liina Hints"
      ],
      "location": "Room 202",
      "time_start": "2026-06-16T09:00",
      "time_end": "2026-06-16T10:45",
      "description": "",
      "website": ""
    },
    {
      "event_group": "workshop",
      "event_format": "tutorial",
      "is_continued": true,
      "track": "track3",
      "theme": "workshop-f57f17",
      "title": "Clear by design – A practical guide to making better maps and figures",
      "short_title": "Map Design",
      "author": [
        "Liina Hints"
      ],
      "location": "Room 202",
      "time_start": "2026-06-16T11:15",
      "time_end": "2026-06-16T13:00",
      "description": "",
      "website": ""
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": false,
      "track": "track4",
      "theme": "workshop-1565c0",
      "title": "Reaping the benefits of open and reproducible research",
      "short_title": "Reproducible Research",
      "author": [
        "Frank Ostermann"
      ],
      "location": "Room 335a",
      "time_start": "2026-06-16T09:00",
      "time_end": "2026-06-16T10:45",
      "description": "",
      "website": ""
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": true,
      "track": "track4",
      "theme": "workshop-1565c0",
      "title": "Reaping the benefits of open and reproducible research",
      "short_title": "Reproducible Research",
      "author": [
        "Frank Ostermann"
      ],
      "location": "Room 335a",
      "time_start": "2026-06-16T11:15",
      "time_end": "2026-06-16T13:00",
      "description": "",
      "website": ""
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Lunch",
      "location": "Vanemuise 46 Gallery",
      "time_start": "2026-06-16T13:00",
      "time_end": "2026-06-16T13:45"
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": false,
      "track": "track4",
      "theme": "workshop-1565c0",
      "title": "Inclusive walkability data through virtual audits: populations and features (InWalkData)",
      "short_title": "InWalkData",
      "author": [
        "Nir Fulman",
        "Johannes Huber",
        "Alexander Zipf"
      ],
      "location": "Room 335a",
      "time_start": "2026-06-16T13:45",
      "time_end": "2026-06-16T15:30",
      "description": "",
      "website": ""
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": true,
      "track": "track4",
      "theme": "workshop-1565c0",
      "title": "Inclusive walkability data through virtual audits: populations and features (InWalkData)",
      "short_title": "InWalkData",
      "author": [
        "Nir Fulman",
        "Johannes Huber",
        "Alexander Zipf"
      ],
      "location": "Room 335a",
      "time_start": "2026-06-16T16:00",
      "time_end": "2026-06-16T17:45",
      "description": "",
      "website": ""
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": false,
      "track": "track2",
      "theme": "workshop-1565c0",
      "title": "Geospatial AI for Urban Analytics (GeoAI4UA) with Actionable Smart Data",
      "short_title": "GeoAI4UA",
      "author": [
        "Meiliu Wu",
        "Jing Yao"
      ],
      "location": "Room 246",
      "time_start": "2026-06-16T13:45",
      "time_end": "2026-06-16T15:30",
      "description": "",
      "website": ""
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": true,
      "track": "track2",
      "theme": "workshop-1565c0",
      "title": "Geospatial AI for Urban Analytics (GeoAI4UA) with Actionable Smart Data",
      "short_title": "GeoAI4UA",
      "author": [
        "Meiliu Wu",
        "Jing Yao"
      ],
      "location": "Room 246",
      "time_start": "2026-06-16T16:00",
      "time_end": "2026-06-16T17:45",
      "description": "",
      "website": ""
    },
    {
      "event_group": "workshop",
      "event_format": "tutorial",
      "is_continued": false,
      "track": "track3",
      "theme": "workshop-f57f17",
      "title": "Hands-on with ESA EarthCode - Using Open EO Research Data",
      "short_title": "EarthCODE",
      "author": [
        "Deyan Samardzhiev"
      ],
      "location": "Room 202",
      "time_start": "2026-06-16T13:45",
      "time_end": "2026-06-16T15:30",
      "description": "",
      "website": ""
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": false,
      "track": "track1",
      "theme": "workshop-1565c0",
      "title": "Uncertain Geodata: Bias, Missingness & GeoAI",
      "short_title": "Uncertain Geodata",
      "author": [
        "James Ackland"
      ],
      "location": "Room 327",
      "time_start": "2026-06-16T13:45",
      "time_end": "2026-06-16T15:30",
      "description": "",
      "website": "https://gdsglasgow.github.io/16-june-or-agile-worksop-uncertain-geodata-bias-missingness-and-geoai/"
    },
    {
      "event_group": "workshop",
      "event_format": "workshop",
      "is_continued": true,
      "track": "track1",
      "theme": "workshop-1565c0",
      "title": "Uncertain Geodata: Bias, Missingness & GeoAI",
      "short_title": "Uncertain Geodata",
      "author": [
        "James Ackland"
      ],
      "location": "Room 327",
      "time_start": "2026-06-16T16:00",
      "time_end": "2026-06-16T17:45",
      "description": "",
      "website": "https://gdsglasgow.github.io/16-june-or-agile-worksop-uncertain-geodata-bias-missingness-and-geoai/"
    },
    {
      "event_group": "workshop",
      "event_format": "tutorial",
      "is_continued": false,
      "track": "track3",
      "theme": "workshop-f57f17",
      "title": "Climate Surface Modelling with Remote and In-Situ Data",
      "short_title": "Climate Modelling",
      "author": [
        "Lluís Pesquer Mayos"
      ],
      "location": "Room 202",
      "time_start": "2026-06-16T16:00",
      "time_end": "2026-06-16T17:45",
      "description": "",
      "website": ""
    },
    {
      "event_group": "social",
      "event_format": "social",
      "track": "",
      "theme": "social-ffffff",
      "title": "Ice-Breaker",
      "location": "Väike Till Event Centre, Aparaaditehas",
      "webpage": "https://sevensons.ee/kohad/vaike-till-sundmuskeskus/",
      "directions": "https://maps.app.goo.gl/MAdtgUFe9Qa2LASE6",
      "time_start": "2026-06-16T19:00",
      "time_end": "2026-06-16T23:00"
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Coffee Break",
      "location": "Vanemuise 46 Gallery",
      "time_start": "2026-06-16T10:45",
      "time_end": "2026-06-16T11:15"
    },
    {
      "event_group": "break",
      "event_format": "break",
      "track": "",
      "title": "Coffee Break",
      "location": "Vanemuise 46 Gallery",
      "time_start": "2026-06-16T15:30",
      "time_end": "2026-06-16T16:00"
    },
    {
      "event_group": "registration",
      "event_format": "registration",
      "track": "",
      "title": "Registration desk open",
      "location": "Vanemuise 46",
      "time_start": "2026-06-16T08:00",
      "time_end": "2026-06-16T09:00"
    }
  ]
}
